...
public:
    int32_t field_70;
    int32_t m_handShakeType;

    static void InjectHooks();

    CTaskComplexPartnerGreet(const char* commandName, CPed* partner, bool leadSpeaker, float distanceMultiplier, int32_t handShakeType, CVector point);
    ~CTaskComplexPartnerGreet() {};
private:
    CTaskComplexPartnerGreet* Constructor(const char* commandName, CPed* partner, bool leadSpeaker, float distanceMultiplier, int32_t handShakeType, CVector point);
public:
