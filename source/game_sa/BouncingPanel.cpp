...
// 0x6F4920
void CBouncingPanel::SetPanel(int16_t frameId, int16_t axis, float angleLimit) {
    m_nFrameId = frameId;
    m_nAxis = axis;
...
void CBouncingPanel::ProcessPanel(CVehicle* vehicle, RwFrame* frame, CVector moveForce, CVector turnForce, float fReturnMultiplier, float fDampMultiplier) {
    CMatrix  frameMat(RwFrameGetMatrix(frame), false);
    CMatrix& vehMat = vehicle->GetMatrix();
    CVector  compPos = Multiply3x3(vehMat, frameMat.GetPosition());
    CVector  velocity = Multiply3x3(vehicle->GetSpeed(compPos) - moveForce + CrossProduct(compPos, turnForce), vehMat);

    switch (m_nAxis) {
    case 0: {
        m_vecPos.y += GetAngleChange(velocity.z);
        m_vecPos.z += GetAngleChange(velocity.y);
        break;
    }
    case 1: {
        m_vecPos.x += GetAngleChange(velocity.z);
        m_vecPos.z += GetAngleChange(velocity.y);
        break;
    }
    case 2: {
        m_vecPos.x += GetAngleChange(velocity.y);
        m_vecPos.y += GetAngleChange(velocity.x);
        break;
    }
    }
