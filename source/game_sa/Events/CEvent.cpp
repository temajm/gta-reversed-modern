...
bool CEventEscalator::AffectsPed_Reversed(CPed* ped)
{
    if (ped->IsAlive() && !ped->IsPlayer() && ped->m_pContactEntity) {
        int32_t modelId = ped->m_pContactEntity->m_nModelIndex;
...
CEventLeanOnVehicle::CEventLeanOnVehicle(CVehicle* vehicle, int32_t leanAnimDurationInMs)
{
    m_vehicle = vehicle;
...
CEventLeanOnVehicle* CEventLeanOnVehicle::Constructor(CVehicle* vehicle, int32_t leanAnimDurationInMs)
{
