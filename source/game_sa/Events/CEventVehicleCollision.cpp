...
CEventVehicleCollision::CEventVehicleCollision(int16_t pieceType, float damageIntensity, CVehicle* vehicle, const CVector& collisionImpactVelocity, const CVector& collisionPosition, int8_t moveState, int16_t evadeType)
{
...
// 0x4AC840
CEventVehicleCollision* CEventVehicleCollision::Constructor(int16_t pieceType, float damageIntensity, CVehicle* vehicle, const CVector& collisionImpactVelocity, const CVector& collisionPosition, int8_t moveState, int16_t evadeType)
{
...
// 0x4B6BC0
CEvent* CEventVehicleCollision::Clone() const
{
...
CEvent* CEventVehicleCollision::Clone_Reversed() const
{
...
bool CEventVehicleCollision::AffectsPed_Reversed(CPed* ped)
{
    ...
    if (pSimplestActiveTask && CTask::IsGoToTask(pSimplestActiveTask)) {
        ...
        int32_t hitSide = CPedGeometryAnalyser::ComputeEntityHitSide(*ped, *m_vehicle);
        if (hitSide == CPedGeometryAnalyser::ComputeEntityHitSide(pGoToTask->m_vecTargetPoint, *m_vehicle)) {
            ...
            int32_t targetPointInPlanes = 0, pedInPlanes = 0;
