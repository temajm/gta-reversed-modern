...
// 0x4F6420
void CAEVehicleAudioEntity::AddAudioEvent(eAudioEvents soundId, float fVolume) {
    plugin::CallMethod<0x4F6420, CAEVehicleAudioEntity*, eAudioEvents, float>(this, soundId, fVolume);
}

// 0x4F7580
void CAEVehicleAudioEntity::AddAudioEvent(eAudioEvents soundId, CEntity* entity) {
    plugin::CallMethod<0x4F7580, CAEVehicleAudioEntity*, eAudioEvents, CEntity*>(this, soundId, entity);
}
...
void CAEVehicleAudioEntity::GetAccelAndBrake(cVehicleParams& vehParams) {
    if (CReplay::Mode == REPLAY_MODE_1) {
        auto* vehicle = vehParams.m_pVehicle->AsVehicle();
...
void CAEVehicleAudioEntity::UpdateGasPedalAudio(CVehicle* vehicle, int32 vehicleType) {
    ...
    float& fSomeGasPedalStuff = ((eVehicleType)vehicleType == VEHICLE_TYPE_BIKE) ? vehicle->AsBike()->m_fGasPedal : vehicle->AsAutomobile()->m_fGasPedal;
...
void CAEVehicleAudioEntity::GetSirenState(bool& bSirenOrAlarm, bool& bHorn, cVehicleParams& params) {
    ...
    auto* vehicle = params.m_pVehicle->AsVehicle();
    ...
    if (vehicle->vehicleFlags.bEngineOn && (vehicle->m_fGasPedal < 0.0f || !vehicle->m_nCurrentGear)) { // Check if we are reversing
        ...
        if (vehicle->m_nWheelsOnGround) {
            ...
            if (vehicle->m_wheelsOnGrounPrev)
                vehicle->m_fGasPedal *= 0.4;
            fReverseGearVelocityProgress = vehicle->m_fGasPedal;
        }
